...
Процедура \procname{variable-matrix} возвращает матрицу $A$ системы
...
процедуру высшего порядка \procname{general-horner-eval}, представляющую
...
процедуры высшего порядка \procname{fold-right}, реализующей свёртку
последовательности справа налево.

Наконец, процедура \procname{matrix-exp} — вычисление по схеме Горнера,
...
процедурой \procname{exp-series-coefficients}
(см. с. \pageref{shared.scm-full-listing}), выраженной в терминах уже
упоминавшейся ранее \procname{evolve-sequence}.
...
\procname{build-fundamentals} в процедуре \procname{approximate-solution}
